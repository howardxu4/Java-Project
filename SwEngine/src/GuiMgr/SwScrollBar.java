 ...
 *
 * SwScrollBar is a class of JScrollBar which configured by XML 
 *
 * </pre>
 *******************************************************************************
 * <B> Author: </B><p><pre>
 ...
 *******************************************************************************
*/
public class SwScrollBar extends JScrollBar implements Iforward
{
	public SwScrollBar (readXML rd, Node pnode) {
		super();
		try {
			...
			for (int i = 0; i < nodeList.getLength(); i++ )
			{
				...
				if (nm.equalsIgnoreCase("Name")) {
					...
					if (listener instanceof AdjustmentListener)
						this.addAdjustmentListener((AdjustmentListener)listener);
				}
				else 
				if (LogTrace.getLog().chkLevel(LogTrace.WARNING)) 
					LogTrace.getLog().Trace(LogTrace.WARNING, 
						"Name " + node.getNodeName() + " is not processed in ScrollBar");				
			}
		}
		catch (Exception e) {
			if (LogTrace.getLog().chkLevel(LogTrace.EXECPT)) 
				LogTrace.getLog().Trace(LogTrace.EXECPT, 
					"ScrollBar Exception:" + e.getStackTrace() + " " + e.toString());
		}
	}
