...
{	
	...
	public boolean setDocument(InputStream stream) {
		try {
			if (builder == null) return false;
...
//			System.out.println("cv = " + node.getFirstChild().getNodeValue().trim());
	...
	private String recDump(NodeList nodeList, boolean f)
	{
		String s = new String();
		String t = new String();
		Vector <Node> trash = new Vector<Node>();
		for ( int i = 0; i < nodeList.getLength(); i++ )
		{
...
*/
		...
		for (Enumeration <Node> e=trash.elements(); e.hasMoreElements();)
		{
